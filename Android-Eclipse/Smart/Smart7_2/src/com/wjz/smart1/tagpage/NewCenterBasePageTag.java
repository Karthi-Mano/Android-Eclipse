...
public class NewCenterBasePageTag extends BasePageTag {
	...
	public void initData() {
		...
		// Json数据缓存处理
		// 1.本地缓存
		String jsonCache = SpTools.getString(mainActivity, MyContainer.URL, "");
		...
		httpUtils.send(HttpMethod.GET, MyContainer.URL,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						//保存json数据到本地
						SpTools.setString(mainActivity, MyContainer.URL,
								jsonData);
						parseJson(jsonData);
					}
	...
	public void swtichPage(int position) {
		...
		currentNewsCenterPage.initData();
		
		//判断 如果是组图 listgrid切换的按钮显示
		if(currentNewsCenterPage instanceof PhotosBaseNewsCenterPage){
			//组图
			//显示listgrid切换的按钮显示
			listorgrid.setVisibility(View.VISIBLE);
			//添加事件，点击做list和grid切换
			//给listorgrid设置一个标签，添加一个额外的数据有getTag取得
			listorgrid.setTag(currentNewsCenterPage);
			listorgrid.setOnClickListener(new OnClickListener() {
				
				@Override
				public void onClick(View v) {
					((PhotosBaseNewsCenterPage)listorgrid.getTag()).switchListViewOrGridView(listorgrid);
