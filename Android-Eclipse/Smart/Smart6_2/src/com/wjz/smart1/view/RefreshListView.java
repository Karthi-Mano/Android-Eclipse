	...
	private void initEvent() {
		...
		this.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				// 状态停止，如果ListView显示最后一条 加载更多数据的显示
				// System.out.println("onScrollStateChangedonScrollStateChangedonScrollStateChangedonScrollStateChanged");
				// 如果是最后一条数据，显示加载更多组件,如果处于加载更多中，不能进入此方法
				if (getLastVisiblePosition() == view.getAdapter().getCount() - 1
						&& !isLoadingMore) {
					ll_listview_foot_root.setPadding(0, 0, 0, 0);
					setSelection(getAdapter().getCount());// 设置最后的内容显示

					//改变加载数据的状态，正在加载数据
					isLoadingMore=true;
					// 调用接口方法
					if (refreshListenre != null) {
						refreshListenre.loadingMore();//实现该接口的组件取完成数据的加载
					}
				}
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				// System.out.println("onScrollonScrollonScrollonScrollonScrollonScroll");
