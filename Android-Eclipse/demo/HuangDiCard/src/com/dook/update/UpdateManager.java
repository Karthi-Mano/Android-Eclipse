...
public class UpdateManager {
	...
	public void checkUpdate() {
		...
		if (info != null) {
				...
				//Toast.makeText(mContext, info.getVersionCode() + " 版本号", Toast.LENGTH_LONG).show();
	...
	private VersionInfo getVersionInfoFromServer() {
		...
		try {
			...
			url = new URL("http://223.4.86.22/file/hdclockversion.xml");//new URL("http://192.168.1.250:838/WebDemo/file/hdclockversion.xml");//
	...
	private Runnable downApkRunnable = new Runnable(){
		...
		public void run() {
			if (!android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {
				...
				try {
					//服务器上新版apk地址 
					URL url = new URL("http://223.4.86.22/file/HuangDiCard.apk");
